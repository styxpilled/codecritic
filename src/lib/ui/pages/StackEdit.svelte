...
<form
	action={edit ? `/user/${$user?.id}/stacks/${stack.id}/edit` : '/stacks/new?/create'}
	method="post"
>
	<div class="col">
		<div class="row">
			<div class="col">
				<label>
					Name
					<input type="text" name="name" required bind:value={stack.name} />
				</label>
			...
			<div class="col">
				<label>
					Description
					<textarea name="description" bind:value={stack.description}></textarea>
				</label>
			</div>
		</div>
		<div class="row">
